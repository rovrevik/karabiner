...
| `06-numpad.json` | Physical left_command (tap: tab) + keys for numpad layer (brackets, numbers). Uses variable `physical_left_command` to distinguish from home-row-mod D |
| `07-sympad.json` | Physical right_command (tap: spacebar) + keys for symbol layer (symbols, punctuation). Uses variable `physical_right_command` to distinguish from home-row-mod K |
...
The combined JSON is built with **rule order** 00, 05, 06, 07, 01, 02, 03, 04 (see `COMBINE_ORDER` in `build.sh`) so that numpad/sympad command-key manipulators are evaluated before 02 disables left_command/right_command.
